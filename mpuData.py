...
class MPU6050:
    ...
    #should we make this private ba? like di makita here?
    #the comments are somehow the explanations, no need to delete them
    ...
    while time.time() - start_time < duration:
        try:
            if show_all:
                ...
                print(f"Temperature (°C):      {data['temp']:.2f}")
                ...
                print(f"Accelerometer Bias:    X={axis_info['accel_bias']['x']:6d}  Y={axis_info['accel_bias']['y']:6d}  Z={axis_info['accel_bias']['z']:6d}")
                print(f"Gyroscope Bias:        X={axis_info['gyro_bias']['x']:6d}  Y={axis_info['gyro_bias']['y']:6d}  Z={axis_info['gyro_bias']['z']:6d}")
                
                if mpu.has_magnetometer:
                    print(f"Magnetometer Bias:     X={axis_info['mag_bias']['x']:6.1f}  Y={axis_info['mag_bias']['y']:6.1f}  Z={axis_info['mag_bias']['z']:6.1f}")
                ...
                print(f"Mapped Accel (g):      X={ax_mapped:7.3f}  Y={ay_mapped:7.3f}  Z={az_mapped:7.3f}")
                print(f"Mapped Gyro (°/s):     X={gx_mapped:7.2f}  Y={gy_mapped:7.2f}  Z={gz_mapped:7.2f}")
